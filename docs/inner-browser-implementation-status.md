...
| Tool | Status | Notes |
| --- | --- | --- |
| `navigate` | Implemented | Supports URL, back, forward, reload. |
| `list_tabs` | Implemented | Lists open browser tabs and active page state. |
| `switch_tab` | Implemented | Selects a tab by `pageId`. |
| `new_tab` | Implemented | Creates a tab, optionally loading a URL. |
| `close_tab` | Implemented | Closes a tab by `pageId`. |
| `show_browser` | Implemented | Shows the right browser panel. |
| `hide_browser` | Implemented | Hides the right browser panel. |
| `evaluate` | Implemented | Executes a JavaScript function in the active webview. |
| `snapshot` | Implemented | Traverses interactive DOM elements and assigns `data-moke-uid` IDs. Returns visible page content through `content.markdown` and actionable elements through `elements`. Supports JSON file output through `filePath`. |
| `screenshot` | Implemented | Captures viewport screenshots, element screenshots by snapshot `uid`, and vertical full-page screenshots to PNG files. |
...
| `wait_for_text` | Implemented | Polls page text until a target string appears. |
| `press` | Implemented | Dispatches synthetic keyboard events to the active element. |
| `type` | Implemented | Appends text to the active editable element, optionally dispatching a submit key. |
| `handle_dialog` | Implemented | Injects a future dialog policy for `alert`, `confirm`, and `prompt`. |
| `resize_viewport` | Implemented | Exposes the existing native webview resize command as an AI tool. |
...
| `press` / `type` | Medium | Synthetic keyboard events are not trusted browser input. |
| `wait_for_text` | High | Uses page text polling. It does not wait on selectors yet. |
...
| `resize_viewport` | Medium | Resizes the embedded webview bounds, not a full device emulation profile. |
| `screenshot` | Medium | Uses native window capture through `xcap`, then crops or stitches browser viewport images. Full-page capture is vertical stitching and may show artifacts on sticky headers, animations, lazy loading, or scroll-snapping pages. |
...
8. `hover`
9. `handle_dialog`
10. `screenshot` viewport, element, and full-page capture
11. `upload_file`
